...
@after {
	...
	(
		...
		{
			$current = $this_Number_13.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
		...
		{
			$current = $this_Literal_14.current;
			afterParserOrEnumRuleCall();
		}
	)
;
...
// Entry rule entryRuleContentUnit
entryRuleContentUnit returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getContentUnitRule()); }
	iv_ruleContentUnit=ruleContentUnit
	{ $current=$iv_ruleContentUnit.current; }
	EOF;

// Rule ContentUnit
ruleContentUnit returns [EObject current=null]
@init {
	enterRule();
}
@after {
...
// Rule ContentUnitType
ruleContentUnitType returns [Enumerator current=null]
@init {
	enterRule();
}
@after {
	...
	(
		(
			enumLiteral_0='Interoperability'
			{
...
@after {
	...
	(
		(
		    ...
		    |
		(
			enumLiteral_1='BackgroundColour'
			{
...
RULE_PRIMITIVETYPES : ('VideoType'|'ImageType'|'TextType'|'GameType'|'IntType'|'Any');
